...
gnosari worker start --concurrency 8

# Scale down for memory-constrained environments
gnosari worker start --concurrency 1
```
...
# Log to file
gnosari worker start --loglevel info > worker.log 2>&1
```
...
gnosari worker start --queue fast_queue --concurrency 8

# Slow queue workers (less concurrency, more memory)
gnosari worker start --queue slow_queue --concurrency 2
```
...
CELERY_WORKER_MAX_TASKS_PER_CHILD=1000 gnosari worker start
```
...
gnosari worker restart --concurrency 2

# Enable worker recycling
CELERY_WORKER_MAX_TASKS_PER_CHILD=500 gnosari worker start
```
...
command=gnosari worker start --concurrency 4
directory=/app
user=gnosari
autostart=true
autorestart=true
